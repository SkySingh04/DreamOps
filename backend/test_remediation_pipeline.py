...
async def test_oom_remediation():
    ...
    try:
        # Get current config and force YOLO mode for testing
        config = get_config()
        original_mode = config.ai_mode
        config.ai_mode = "YOLO"
        config.ai_confidence_threshold = 0.7  # Lower threshold for testing
        
        print("\n🤖 AI Mode: YOLO (autonomous execution)")
